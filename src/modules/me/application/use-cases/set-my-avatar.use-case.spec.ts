...
function makeDeps(currentAvatarUrl: string | null) {
  ...
  const storage: FileStoragePort = {
    save: async (ns, filename) => {
      calls.saved.push(`${ns}/${filename}`);
      return { url: `http://h/api/v1/files/${ns}/${filename}` };
    },
...
describe('SetMyAvatarUseCase', () => {
  ...
  it('deletes the previous avatar (different ext) before saving', async () => {
    const { calls, repo, storage } = makeDeps(
      'http://h/api/v1/files/avatars/u1.jpg',
    );
  ...
  it('rejects an unsupported content type', async () => {
    ...
    await expect(
      new SetMyAvatarUseCase(repo, storage).execute({
        userId: 'u1',
