...
describe('PrismaUserProfileRepository (int)', () => {
  ...
  beforeAll(async () => {
    repo = new PrismaUserProfileRepository(new PrismaService(new TenantContextService()));
    ...
    const user = await raw.user.create({
      data: { email: 'perfil-repo@test.com', passwordHash: 'HASH0', fullName: 'Antes' },
      select: { id: true },
    });
    userId = user.id;
  });
  ...
  it('reads the user, updates name/password/avatar and reads the clinic name', async () => {
    ...
    expect(before).toMatchObject({ email: 'perfil-repo@test.com', fullName: 'Antes', avatarUrl: null });

    await repo.updateName(userId, 'Después');
    await repo.updatePasswordHash(userId, 'HASH1');
    await repo.updateAvatarUrl(userId, 'http://files.test/api/v1/files/avatars/x.png');

    const after = await repo.findUserById(userId);
    expect(after?.fullName).toBe('Después');
    expect(after?.avatarUrl).toBe('http://files.test/api/v1/files/avatars/x.png');
    expect(await repo.getPasswordHash(userId)).toBe('HASH1');
  });

  it('returns null for an unknown user', async () => {
    expect(await repo.findUserById('00000000-0000-0000-0000-000000000000')).toBeNull();
  });
});
